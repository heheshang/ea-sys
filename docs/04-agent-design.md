...
**策略决策**（AssistantPolicy，确定性规则，分支顺序）：取消词 → 不调工具直接收尾；政策值问句（目标/达标线/基线/阈值 + 数值问法）→ 优先知识库检索 search_kb，而非实时指标 query_stats；知识库命中 → 摘要 + 引用卡；stats / 人群 / 工作流列表意图 → 各查询工具并出卡；创建意图 → `begin_workflow_dialogue` 工具，成功后发自定义事件 `switch_workflow_dialogue`，前端切会话并自动发起创建；触发意图 → `search_workflows` 必经 + `trigger_workflow`（框架 ASK → HITL 确认卡）；其余 → 直接文本回复（可闲聊）。
...
## 13. 评测中心（五层架构：数据集 / 执行 / 记录 / 评分 / 聚合）

**职责**：按蓝图五层（数据集层 / 执行层 / 记录层 / 评分层 / 聚合层）组织的评测中心 —— 数据集层承载分层用例（basic / edge / real）、逐用例判分规则（judge_rule）、多轮对话（dialogue）与不可变发布版本（evaluation_dataset_version）；执行层真实运行被测智能体（openjudge 预置响应；execute 单轮 / 多轮共享会话）；记录层持久化完整执行轨迹（evaluation_transcript，逐样本含 latency_ms）；评分层三类判分（确定性规则 / LLM-Judge / Human 黄金标准）→ 各评测器均值；聚合层产出报告三快照（env / code / 数据集版本）、执行统计、分层统计、上线建议（GO / WATCH / NO_GO）与看板。内置评测器是代码常量目录（规则 11 + LLM-Judge 6 = 17）不落表；自定义评测器落 `evaluation_custom_evaluator` 表（rule = Java 参数化规则 / llm_judge = 可配提示词判分，不引 Python）。openjudge 用预置响应判分；execute 由 service 真实运行被测智能体（assistant / workflow-dialogue，`ReActAgent.call` 同步 + 30s 超时），注入实际回复与工具调用轨迹（`actual_tool_calls` / `actual_steps`）后走同一判分链路。
...
| 端点 | 说明 |
|---|---|
| `GET /api/evaluations/datasets` | 数据集列表（含 caseCount；?includeDraft=true 扩展 latestVersionId/latestVersionNo/caseCountByCategory） |
...
| `GET /api/evaluations/datasets/{id}/cases` | 用例列表（seq 升序，含 category / judge_rule / dialogue） |
| `POST /api/evaluations/datasets/{id}/cases` | 新增用例（seq 缺省 = max+1；category / judge_rule / dialogue 可配） |
...
| `POST /api/evaluations/datasets/{id}/versions` | 发布新版本：快照当前工作区用例 + 评测器选择 → versionNo = max+1；无用例变更 → 400 |
| `GET /api/evaluations/datasets/{id}/versions` | 版本列表（versionNo / status / caseCount / publishedAt / createdBy） |
| `GET /api/evaluations/datasets/{id}/versions/{versionId}/cases` | 版本内用例快照（只读，JSON 解析返回 CaseView 列表） |
| `DELETE /api/evaluations/datasets/{id}/versions/{versionId}` | 删除未引用版本；被报告 / 任务引用 → 400 |
...
| `POST /api/evaluations/run` | 批量运行：`{datasetId, datasetVersionId?, evaluators?, judgeRounds?}`（datasetVersionId 缺省 = 最新 PUBLISHED，未发布回退实时并标注；evaluators 缺省 = 全量内置；judgeRounds 1-5 缺省 1，LLM-Judge 多次取均值） |
| `POST /api/evaluations/tasks` | 异步评测任务：同 run 入参 + optional datasetVersionId，202 Accepted + PENDING 任务视图，后台执行（状态机 PENDING→RUNNING→COMPLETED/FAILED/CANCELED） |
| `GET /api/evaluations/tasks` | 任务列表（created_at 倒序，含 progress / sampleResults） |
| `GET /api/evaluations/tasks/{id}` | 任务详情：任务视图 + 逐样本指标聚合（均值 / 通过数 / 适用数 / 离散度 stddev+mads） |
| `GET /api/evaluations/tasks/{id}/transcript?caseSeq=` | 任务执行中逐样本完整转录（轮次升序：[{turnNo, role, text, thinking, toolUse, toolResult}]） |
...
| `GET /api/evaluations/reports` | 报告列表（含 judgeRounds / traceId / datasetVersionNo） |
| `GET /api/evaluations/reports/{id}` | 报告详情（含 datasetVersionId/No、execution / layering / envSnapshot / codeSnapshot、summary.recommendation / top_regressions） |
| `GET /api/evaluations/reports/{id}/transcript?caseSeq=` | 报告逐样本完整转录（同任务转录，轮次升序） |
| `GET /api/evaluations/reports/{id}/compare?baseline=&layer=` | 报告对比：delta=current-baseline（round4），metric 按名称对齐、缺项 null，direction=higher_is_better；新增 layer=basic/edge/real 过滤 + topDegradedSamples[]（按 seq 对齐、|delta| 降序前 N 样本）；缺 baseline → 400 |
| `POST /api/evaluations/reports/{id}/rerun` | 复现：按原报告数据集版本 + 评测器（report.metrics）+ judge_rounds 重跑，新报告基线锚定原报告；数据集版本已软删 → 400 |
| `POST /api/evaluations/reports/{id}/reviews` | 提交 / 改判人工复评 `{caseSeq, metric?, score(0-1), verdict?, note?}`（upsert：软删旧行 + 插新行，UNIQUE 兜底） |
| `GET /api/evaluations/reports/{id}/reviews` | 复评列表 |
| `GET /api/evaluations/reports/{id}/reviews/calibration` | 校准对比：per-metric {n, meanAuto, meanHuman, meanAbsDiff, agreementRate, topDeltas[]}；metric='*' 纯人工整分（无 auto 对齐，仅黄金标准通过率） |
| `DELETE /api/evaluations/reviews/{id}` | 删除单条复评 |
| `GET /api/evaluations/dashboard?datasetId=&limit=` | 看板聚合：{layering, trend, metrics, regressions, costLatency}（limit 默认 12、最多 30） |
| `GET /api/evaluations/catalog` | 评测目录：内置 17 指标（规则 11 + LLM-Judge 6；metric/category/higherIsBetter/defaultThreshold=0.8）+ 启用的自定义评测器（custom_{id}，id 升序） |

**数据集层（分层 + 三要素 + 不可变版本）**：`evaluation_case` 增 `category`（basic/edge/real，NOT NULL DEFAULT 'basic' + CHECK，旧数据自动归 basic 兼容）；运行期按 40/30/30 目标校验分布 —— 参与用例任一档占比 <20% 且用例数 ≥5 → `findings` 增 WARNING「分层偏差」（建议性告警不阻断）。`judge_rule` JSONB 补齐「三要素」中的 JudgeRule：形态 A `{metric, threshold?}`（单评测器 + 阈值覆盖）、形态 B `{metric, judge_prompt?, rounds?, threshold?}`（LLM-Judge 提示词 / 轮次覆盖）、形态 C 数组（逐用例多指标白名单）；判分管线：case.judge_rule 存在且 metric 在白名单 → 该用例仅按 judge_rule 判分并应用其阈值，否则沿用运行级多选（case 级优先、运行级兜底）。`dialogue` JSONB 承载多轮输入（[{role: user|assistant, content, tool_use?, tool_result?}]，单轮为 NULL，question 即首轮内容）。**版本化（immutable + versioned）**：新表 `evaluation_dataset_version`（tenant_id/dataset_id/version_no UNIQUE、status=PUBLISHED、cases + evaluators + published_at，一行 JSONB 快照天然不可变）；「发布」= 快照当前工作区用例与评测器选择，工作区（实时表）继续可编辑；run/task 绑定 datasetVersionId 锁版本复现（缺省 = 最新 PUBLISHED，未发布 → 回退实时并标注），旧数据集迁移自动回填 v1 快照、旧报告回填 version=1 —— 历史报告与新报告同口径可比。

**执行层（标准 Harness + 多轮交互）**：多轮 = dialogue 轮次顺序调同一 `ReActAgent.call`（同一 sessionId 共享 AgentState 会话历史，工具态跨轮保留；单轮用例 dialogue 为 NULL 走现状路径）；sessionId = `traceId-seq` 天然防串扰，总耗时 = 轮数 × 单轮（30s 超时）。HITL 工具（trigger_workflow 等）在无人值守评测无确认响应 → 挂起至超时 → 该用例判分不适用（INFO）；「评测权限自动裁决策略」（设计 E4）尚未实现，记为设计决策留待后续增强 —— 生产 controller 路径不受评测会话影响。

**记录层（完整 Transcript）**：新表 `evaluation_transcript`（report_id / case_seq / turn_no / role（USER/ASSISTANT/SYSTEM/TOOL）/ text / thinking / tool_use JSONB{name, args} / tool_result JSONB{name, state, output}）；采集点 = executeSubject 对被测 `AgentState.getContext()` 每轮增量序列化（与 actual_tool_calls/actual_tool_results 同源同刻），截断 thinking / tool args 4000、tool output 8000（与既有轨迹口径一致）；序列化失败旁路告警，不影响判分主链路。每样本 latency_ms = execute 逐用例起止计时（openjudge → null）。端点：`GET /reports/{id}/transcript?caseSeq=` 与 `GET /tasks/{id}/transcript?caseSeq=`（轮次升序，任务执行中可查）。判分输入升级：多轮用例 LLM-Judge 的 `response` = 各轮 ASSISTANT text 按轮拼接（每轮 ≤2000、总 ≤8000 截断），`reference` 仍取 expected_output；规则类指标（tool_call_accuracy / step_efficiency / policy_compliance / rag_hit_rate / decision_accuracy）判分对象 = 全轮轨迹（actual_tool_calls / actual_steps 跨轮累计，语义不变）。

**评分层（确定性规则 / LLM-Judge / Human 黄金标准）**：code-based 内置规则由 10 条增至 **11**，新增 `decision_accuracy`（决策准确率，execute 专属）：只判首意图决策 —— 首个 executed 工具调用与 expected_tool 匹配，参数全匹配 1.0 / 命中 0.5 / 无调用 0，与 tool_call_accuracy（全轨迹视角）互补；ALL_METRICS 16→**17**（规则 11 + LLM-Judge 6），双端注册（EvaluationModel 常量 + EVALUATOR_CATALOG）。LLM-Judge：case.judge_rule 可携带 judge_prompt / rounds / threshold 逐用例覆盖（injectJudgeScores 透传，LlmJudgeScorer 签名不变）；judgeRounds 均值 + stddev/mads 离散度保留；LLM 未启用降级语义不变。Human 黄金标准（第三类）：新表 `evaluation_human_review`（report_id/case_seq/metric/score(0-1)/verdict/note/reviewer/updated_at，UNIQUE(report_id, case_seq, metric) 部分唯一 —— V19 修复与软删墓碑冲突，仿 V7 先例）；复评提交 / 列表 / 删除端点见上，校准端点输出 per-metric {n, meanAuto, meanHuman, meanAbsDiff, agreementRate（|auto−human|≤0.5 视为一致）, topDeltas[]}，`metric='*'` 纯人工整分（黄金标准通过率）；人工复评不修改报告自动分（对照回流，供 judge 提示词迭代）。

**聚合层（执行统计 + 快照 + 报告驱动 + 看板）**：`evaluation_report` 增列 dataset_version_id / dataset_version_no（数据版本锁）+ env_snapshot {app_version, java_version, llm_enabled, llm_model_id, agent_models} + code_snapshot {git_commit, git_branch, build_time}（构建期注入，缺省回退 app version + 时间）+ execution {avg_latency_ms, p50_latency_ms, p95_latency_ms, avg_steps, total_steps, llm_calls, input_tokens, output_tokens, judge_calls, judge_input_tokens, judge_output_tokens, estimated_cost_cny}（llm_usage 按 session_id=`traceId%`（被测 subject）与 agent_type='evaluation'（judge）聚合，LLM 未启用 → 0 / null，前端显示「—」）+ layering {basic/edge/real: {count, tested, pass_rate}}。`summary` 扩展 recommendation（GO/WATCH/NO_GO + reason）：任一核心指标（task_success / tool_call_accuracy / decision_accuracy / policy_compliance）<0.6 或相对基线回归 >0.1 → NO_GO；任一 <0.8 或回归 >0.05 → WATCH；其余 → GO；top_regressions = {metrics: [{metric, current, baseline, delta}], samples: [{seq, auto, baseline, delta}]}（无 baseline → 空）。看板端点 `GET /api/evaluations/dashboard?datasetId=&limit=`：{layering（三档分布）, trend（报告序列 summary score/verdict + execution）, metrics（核心指标 series/latest/delta）, regressions（topDegraded）, costLatency（avg_latency_ms / p95_latency_ms / avg_steps / total_tokens / cost_cny）}。

**回归与可追溯**：compare 增强（T1）—— 既有 `?baseline=` delta 口径完全兼容，新增可选 `layer=basic|edge|real` 过滤 + 响应 `topDegradedSamples[]`（按 seq 对齐、|delta| 降序前 N 样本，含 seq/auto/baseline）；复现端点 `POST /reports/{id}/rerun`（T3）—— 按原报告数据集版本 + 评测器 + judge_rounds 重跑，新报告 baseline_id 指向原报告（一键复现自带基线），数据集版本已软删 → 400；可复现三件套 = 数据版本快照 + env_snapshot + code_snapshot + traceId。报告详情五区块（总览 recommendation 徽章 / 分层卡 / 执行统计 / Top 退化样例（点击进样本抽屉看 transcript）/ 复现）。

**前端**（V18）：数据集表分层分布色条列（el-progress 三段 40/30/30 高亮）+ 发布版本弹窗 + 版本快照只读；运行表单版本下拉（默认最新已发布）+ datasetVersionId 入参；用例表单 category 下拉 / judge_rule JSON / dialogue 轮次编辑；样本抽屉 transcript 逐轮视图 + latency；报告详情五区块 + 复评抽屉 + 校准抽屉；对比三 tab（指标 / 分层 / Top 退化）；看板区块（Element Plus，无图表依赖）；EVALUATOR_CATALOG 17 条（规则 11 + LLM-Judge 6）双端对齐。

**异步任务**（V16）：`evaluation_task` 表承载状态机（PENDING / RUNNING / CANCELING / COMPLETED / FAILED / CANCELED，软删）。执行线程在 `TenantContext` 上下文内逐用例调确定性 `EvaluationModel.plan()`（与批量 `build()` 数学等价，引擎零改动），每 5 例 `markProgress` 更新 `tested_cases` 与 `progress_pct`（2 位小数）；LLM-Judge 判分 `injectJudgeScores` 逐用例注入，`sample_results` JSONB 以 `{seq, question, actual_response, latency_ms, mode, metrics[{metric, category, score, passed, reason?, round_scores?}]}` 落库（reason 取首个非空判分理由截 500，round_scores 为多轮判分分数四舍五入整数数组，离散度 stddev/mads 供详情聚合）；task 与 report 共享 traceId（params 快照读取，含 datasetVersionId 标注），终态写入 Report 且任务 `report_id` 回填，报告结构与同步 run 完全一致。取消竞态由 SQL 状态前置裁决（`WHERE status='RUNNING'` 等），不会双写报告；FAILED 仅任务级（数据集停用 / 无可评测用例 / 未知 mode / 异常），失败样本语义不变。任务审计：create → `EVALUATION_TASK_CREATE`、cancel → `EVALUATION_TASK_CANCEL`，run 审计仍只写一次 `evaluation_run`（SUCCESS/rule）且为最后一条。

**评测目录（与 `EvaluationModel` 常量逐字对齐）**：规则 11 —— `number_accuracy` / `string_exact` / `response_repetition` / `text_similarity` / `observation_information_gain` / `tool_call_accuracy` / `task_success` / `step_efficiency` / `policy_compliance` / `rag_hit_rate` / `decision_accuracy`；LLM-Judge 6 —— `llm_correctness` / `llm_instruction_following` / `llm_relevance` / `llm_hallucination` / `llm_reasoning_groundedness` / `llm_response_completeness`。四个执行维度评测器参考通用 agent 评测指标：工具调用正确性（期望工具名精确命中 0.5 / 参数全匹配 1.0，有轨迹但未调用 0）、端到端任务成功（期望数字全集命中或文本 bigram 相似度 ≥0.8）、步骤效率（min(1, 期望步数/实际步数)）、策略合规（`expected_policy=[{keyword,prohibit}]` 必备/禁区词，任一违规 0）；decision_accuracy 是「首意图决策」视角（首个 executed 工具调用），与全轨迹的 tool_call_accuracy 互补。模型入参不在白名单（内置 `ALL_METRICS`（17 条）或本次运行已启用的自定义 `custom_{id}`）的 metric 静默丢弃，前端目录必须与之对齐。rag_hit_rate（V17，execute 专属）= search_kb 检索命中率：用例 expected_kb_hits 期望片段与实际检索结果（documentName+content 拼接）做字符 bigram 重合，≥0.5 判中；openjudge / 未调用 search_kb / hits 空 → 不适用（INFO）。
...
**判分与报告**：判分对象 `actual_response`（openjudge 由 service 复制 provided_response；execute 注入真实回复与轨迹；空 = 不适用 null，不计均值）；execute 轨迹现采集 actual_tool_calls（名+参数）与 actual_tool_results（名/状态/输出，截 8000），判分输入含 expected_kb_hits；单用例得分 ≥0.8 记通过；`metrics[] {metric, category, avg_score, passed_count, applicable_count, stddev?, p50?, p95?}` 只含适用用例数 >0 的评测器，无适用用例的评测器不进 metrics、仅以 `findings[]` INFO 发现列出（jsonb 原文 snake_case），`summary {score, verdict, recommendation?, top_regressions?}`（≥80 PASS / ≥60 WARN / 其余 FAIL；recommendation = GO/WATCH/NO_GO 见聚合层），`findings[]` 含 INFO / WARNING / BLOCKED（均值 <0.6）分级与修复建议。报告落 `evaluation_report`（jsonb 原样透传），顶层 `testedCases/totalCases` 走 JavaBean 序列化驼峰，新增 `judgeRounds` / `traceId` / `datasetVersionId` / `datasetVersionNo` / `envSnapshot` / `codeSnapshot` / `execution` / `layering` 列。execute 单用例失败 / 超时 / 空回复不整轮报错，该用例判分不适用（INFO），审计形状与 openjudge 一致（同一 `AgentPolicy.run` + `persistAudit`）。
...
**审计约定**：图谱 / 数据集 / 用例 / 导入 / 自定义评测器 / 报告的全部写操作（create/update/delete/run/import）均写入 `agent_audit`（action = 端到端动作名，schema 合规即有效），运行评测触发完整 HarnessAgent 链路（超时 / 降级兜底齐全）；前端 `POST /run` 前检查数据集 enabled、用例数 >0，execute 模式同样可运行（数据集对话框选择被测智能体）。评测运行审计的 `input_summary` 顶层含 `trace_id` / `judge_rounds` / `custom_evaluators`（V18 起含 `dataset_version_id` / `category` 分布标注），供驾驶舱按追踪 ID 联动过滤。