...
/**
 * AI 智能客服：确定性模型位（同 WorkflowDialogueModel 模式）。
 *
 * <p>不依赖任何 LLM：读全量会话历史 → {@link AssistantPolicy#decide} 决策 →
 * 组装一个 ChatResponse（多个 TextBlock 逐句分片 + 末尾一个或多个 ToolUseBlock 工具意图）。
 * 文本块被框架逐块 emit TextBlockDeltaEvent → 前端打字机；工具块进入 ReAct 循环，
 * trigger_workflow 经工具 checkPermissions(ASK) 走 HITL 人工确认闸门。
 *
 * <p>接入真实 LLM（如 ultrathink）时：框架同一模型位换 ModelRegistry 解析出的模型，
 * 对话流程/HITL/卡片语义零改动 —— 工具与 HITL 均为框架级能力。
 */
public class AssistantModel extends ChatModelBase {
    ...
    protected Flux<ChatResponse> doStream(List<Msg> msgs, List<ToolSchema> tools, GenerateOptions options) {
        ...
        try {
            AssistantPolicy.Action action = AssistantPolicy.decide(msgs);
    ...
    private static List<ContentBlock> buildBlocks(AssistantPolicy.Action action) {
        List<ContentBlock> blocks = new ArrayList<>();
        switch (action) {
            case AssistantPolicy.Reply r -> {
                for (String chunk : r.chunks()) {
                    blocks.add(TextBlock.builder().text(chunk).build());
                }
            }
            case AssistantPolicy.Tool t -> {
                for (String chunk : t.prefaceChunks()) {
                    blocks.add(TextBlock.builder().text(chunk).build());
                }
                for (AssistantPolicy.Call c : t.calls()) {
                    blocks.add(toolCall(c.name(), c.input()));
                }
            }
        }
        return blocks;
    ...
    private static ToolUseBlock toolCall(String name, Map<String, Object> input) {
        ...
        // content 解析），input 才是实际执行参数——两者都写，工具的必填参数校验才能通过。
        String contentJson;
        try {
