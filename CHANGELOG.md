...
- AI 智能客服：右下角悬浮窗，知识库问答（RAG 确定性检索 + 引用）、运营数据问答（到达率/留存率/漏斗/工作流效果）、人群检索、工作流触发（HITL 人工确认）、对话式创建工作流一键载入画布（复用 ai-chat 会话）
- 知识库文档管理：上传（txt/md/csv/xlsx/docx/pdf ≤10MB）同步分块入库，列表/删除，CJK 分词 + BM25 引用检索
