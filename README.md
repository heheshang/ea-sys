...
| M1 | contact + audience 人群圈选 API | 待开始 |
| M2 | DAG 工作流引擎（节点 / 触发 / 条件 DSL / 治理）| 待开始 |
| M3 | 触达执行（短信 / 邮件适配器、幂等、频率控制、回执）| 待开始 |
| M4 | 智能体（分层 / 路由 + 计划导入校验 Agent）| 待开始 |
| M5 | 留存看板、流失预警、AB / 灰度 | 待开始 |
...
M0 工程骨架完成：五模块（common / engine / agent / channel / api）+ Flyway 基线（tenant / sys_user）+ 租户上下文 + JWT 鉴权 + empty 链路冒烟通过。
内置开发账号：`admin / admin123`（dev profile 自动初始化租户 1）。
